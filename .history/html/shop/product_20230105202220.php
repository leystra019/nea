...
if(isset($_POST['add_to_bag'])) {
    ...
    $select_bag = mysqli_query($conn, "SELECT * FROM bag WHERE id = '$product_id'");
    if(mysqli_num_rows($select_bag) > 0){
        echo 'product has already been added to bag';
    }else {
        $insert_product = mysqli_query($conn, "INSERT INTO bag (title, price, image, quantity) VALUES ('$product_id','$product_title', '$product_price', '$product_image', '$product_quantity')");

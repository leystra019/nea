<?php
    ...
    if (!isset($_COOKIE['user_id'])) {
        // set the cookie
        $cookie_user_name = 'user_id';
        setcookie($cookie_user_name, $user_id, time() + 86400);
        echo $user_id; 
    } else {
        // the cookie is already set, so we do nothing
    }
    ...
    if(isset($_POST['add_to_bag'])) {
        ...
        if(mysqli_num_rows($select_bag) > 0){
            ...
            // $insert_product = mysqli_query($conn, "INSERT INTO bag (id, product_id, title, price, image, quantity) VALUES ($user_id, ".(int)$productId.", '$product_title', '$product_price', '$product_image', '$product_quantity')");

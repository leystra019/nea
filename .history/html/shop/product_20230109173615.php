<?php
    ...
    if(isset($_POST['add_to_bag'])) {
        ...
        $current_bag = $_COOKIE['bag'];
        // if the bag item is null, we haven't added any products
        if (is_null($current_bag)) {
        // so create a new array, with the only item in there being the item we first had
            $current_bag = array($productId);
        } else {
        // if not empty, then append to existing 
            array_push($current_bag, $productId);
        }
        SETCOOKIE("bag", array(product_id)" ");

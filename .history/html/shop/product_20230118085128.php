<?php
    ...
    if(isset($_POST['add_to_bag'])) {
        ...
        if(mysqli_num_rows($select_bag) > 0){
            $repeatpurchase = "You have already added this item to bag, doing so will update the quantity";
            echo "<script type='text/javascript'>alert('$repeatpurchase');</script>";
        }else {
            if ($user_id != 0) {
                if (mysqli_num_rows($result) > 0) {
                    $user_id = (int)$_COOKIE['user_id'];
                    ...
                    $purchasesuccess = "product has succesfully been added to bag";
                    echo "<script type='text/javascript'>alert('$purchasesuccess');</script>";
                }

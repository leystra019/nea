<?php
    ...
    if (mysqli_stmt_execute($stmt)) {
      ...
      foreach ($product_ids as $product_id) {
        $sql = "INSERT INTO orders (order_id, product_id, quantity, price, status) VALUES (?, ?, ?, ?, ?)";
        $stmt = mysqli_prepare($conn, $sql);
        $product_quantity = $bag[$product_id];
        mysqli_stmt_bind_param($stmt, "iiddi", $order_id, $product_id, $product_quantity, $price, $status);
        ...
        // we need to free the result returned by the previous quert before executing the update
        mysqli_free_result($get_price_stmt);

<?php
    ...
    if (isset($_SESSION['session_auth']) && $_SESSION['session_auth'] == 0) {
        ...
        if (isset($_COOKIE['bag'])) {
            ...
            // We need to set the contents of the bag to a variable
            $current_bag_keys = array_keys($current_bag);
            $checkout_disabled = false;
            //Run a statement to get each product in the bag
            //Bind parameters are used to prevent sql injection
            foreach ($current_bag_keys as $product_id) {
                ...
                // We then need to check if the user is checking out an invalid number of products that matches the database
                // We display the right error depending on too much or too litlle
        ...
        // We can remove item from bag on a button press
        ...
        // If user clicks update, we need to refresh the page
                            ...
                            <?php
                                if (isset($_COOKIE['bag'])) {
                                    ...
                                    while ($row = mysqli_fetch_array($result)) {
                                        ...
                                        echo "<td><img src='" . $row['image'] . "' width='86' height='91.5'></td>";

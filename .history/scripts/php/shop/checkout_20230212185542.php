<?php
    ...
    if (mysqli_stmt_execute($stmt)) {
      ...
      foreach ($product_ids as $product_id) {
        $sql = "INSERT INTO orders (user_id, order_id, product_id, quantity, status) VALUES (?, ?, ?, ?, ?)";
        $stmt = mysqli_prepare($conn, $sql);
        $product_quantity = $bag[$product_id];
        mysqli_stmt_bind_param($stmt, "iiidi", $user_id, $order_id, $product_id, $product_quantity, $status);
        mysqli_stmt_execute($stmt);

        // update the product stock quantity
        $sql = "UPDATE product_stock SET stock_quantity = stock_quantity - ? WHERE product_id = ?";
        $stmt = mysqli_prepare($conn, $sql);

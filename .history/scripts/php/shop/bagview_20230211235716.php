    ...
    if ($_SESSION['session_auth'] == 0) {
        ...
        if (isset($_COOKIE['bag'])) {
            ...
            foreach ($current_bag_keys as $product_id) {
                $sql = "SELECT quantity, title FROM product_stock WHERE product_id = ?";
                $stmt = mysqli_prepare($conn, $sql);
                mysqli_stmt_bind_param($stmt, "i", $product_id);
                mysqli_stmt_execute($stmt);
                $result = mysqli_stmt_get_result($stmt);
                $row = mysqli_fetch_assoc($result);
                if ($current_bag[$product_id] > $row['quantity']) {
                    echo '<div class="alert alert-danger">Sorry, there is only ' . $row['quantity'] . ' available for product ' . $row['title'] . '. Please adjust your quantity in the bag.</div>';
                    echo '<script>$("#checkout-btn").prop("disabled", true);</script>';
                }
              }
            } else {
              echo '<div class="alert alert-warning">Your bag is empty. Please add some items to proceed to checkout.</div>';
              echo '<script>$("#checkout-btn").prop("disabled", true);</script>';
            }
        }

<?php
    ...
    if (mysqli_stmt_execute($stmt)) {
      ...
      $order_id = mt_rand();
      $status = 1;
      $sql = "INSERT INTO orders (user_id, order_id, quantity, status) VALUES (?, $order_id, ?, $status)";
      $stmt = mysqli_prepare($conn, $sql);
      $product_ids = implode(',', array_keys($bag));
      $quantities = implode(',', $bag);
      mysqli_stmt_bind_param($stmt, "iiii", $user_id, $order_id, $quantities, $status);

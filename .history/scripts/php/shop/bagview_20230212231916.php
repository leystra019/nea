<?php
    ...
    if ($_SESSION['session_auth'] == 0) {
        ...
        if (isset($_GET['update'])) {
            // Get the product ID and the new quantity from the URL
            $product_id = $_GET['product_id'];
            $new_quantity = $_GET['new_quantity'];
            
            // Update the quantity of the item in the shopping bag
            $current_bag[$product_id] = $new_quantity;
            
            // Store the updated shopping bag back to the session
            $_SESSION['bag'] = $current_bag;
            
            // Refresh the page to show the updated shopping bag
            header("Refresh:0");
            exit;
        }
                            ...
                            <?php
                                if (isset($_COOKIE['bag'])) {
                                    ...
                                    while ($row = mysqli_fetch_array($result)) {
                                        ...
                                        echo "<td><a class='button' href='/neatest/scripts/php/shop/bagview.php?update=1&product_id=".$row['product_id']."&new_quantity=".$new_quantity."'>update</a><a class='button' href='/neatest/scripts/php/shop/bagview.php?remove=".$row['product_id']."' onclick='return confirm(\"remove item from bag?\")'>delete</a></td>";
                                        echo "</tr>";

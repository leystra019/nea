<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $query = "UPDATE product_stock SET title = ?, brand = ?, description = ?, category = ?, subcategory = ?, price = ? and stock = ? WHERE product_id = ?";
        $stmt = mysqli_prepare($conn, $query);
        mysqli_stmt_bind_param($stmt, "sssssdi", $quantity, $price, $status, $order_id, $product_id);
